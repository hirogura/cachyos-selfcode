...
#
# GitHub (https://github.com/hirogura/cachyos-selfcode) からソースを取得して
...
REPO_URL="https://github.com/hirogura/cachyos-selfcode.git"
BRANCH="main"
...
for cmd in git curl node npm; do
  command -v "$cmd" >/dev/null 2>&1 || die "'$cmd' が見つかりません。先にインストールしてください (例: sudo pacman -S --needed --noconfirm git curl nodejs npm)"
done
