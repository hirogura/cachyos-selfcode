...
const GithubPanel = (() => {
  ...
  function repoItem(r) {
    ...
    div.innerHTML =
      ...
      '<button class="btn small" data-act="force-push" title="git push --force-with-lease origin main（リモートに自分以外の新しいコミットがある場合は失敗する安全な強制push）">強制push</button>' +
  ...
  function onReposClick(e) {
    ...
    if (btn.dataset.act === "force-push") {
      const ok = confirm(
        "強制pushを実行しますか？\n\n" +
        "・git push --force-with-lease origin main を実行します\n" +
        "・リモートに自分以外の新しいコミットがある場合は失敗します（安全）\n" +
        "・リモートの main ブランチがローカルの内容で上書きされます（元に戻せません）"
      );
      if (!ok) return;
