...
// openssh-server を pacman / apt でインストールする（未導入時に y が選ばれた場合）
async function installOpensshServer(target) {
  const cmd =
    ...
    'else echo "[selfcode] 対応するパッケージマネージャが見つかりません（手動で openssh-server を導入してください）" >&2; exit 1; fi';
...
// 認証は登録済みのユーザー名と PAT を使う（ユーザー名が未登録の場合は x-access-token をフォールバックに使う）。
...
app.post("/api/github/repos/:id/action", async (req, res, next) => {
  try {
    ...
    if (!["status", "fetch", "pull", "log", "commit", "cancel", "push", "branch", "cleanup", "force-pull", "force-push"].includes(action)) {
    ...
    if (action === "force-push") {
      // 登録済みのユーザー名と PAT で強制push する。--force-with-lease なので、
      // リモートに自分以外の新しいコミットがある場合は失敗して安全に止まる。
...
curl -fsSL https://raw.githubusercontent.com/hirogura/cachyos-selfcode/main/install-selfcode.sh -o "$update_tmp"
bash "$update_tmp"
`;
